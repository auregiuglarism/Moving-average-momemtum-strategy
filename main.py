...
from config import DEBUG_MAIN, DEBUG_MAIN_ABNORMAL, rebalancing_filter, rebalancing_portfolios, advanced_scoring, dates, binary_gate
...
if __name__ == "__main__":
    ...
    for i in range(len(dates)-1):
        ...
        if pf_long is not None: # Else go to next timeframe, continue the loop
            ...
            print("r_long:", r_long, "r_ls:", r_ls, "r_mim:", r_mim) if DEBUG_MAIN_ABNORMAL else None
...
sp500_resampled = sp500df_copy['Change %'].resample(f'{rebalancing_portfolios}D').sum()  # sum of returns over the rebalancing period
sp500_cumulative = (1 + sp500_resampled).cumprod()
sp500_cumulative = sp500_cumulative.reindex(cumulative_df.index, method='ffill')
